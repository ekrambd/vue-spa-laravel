<template>
    <div class="container-fluid" id="container-wrapper">
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Purchase Logs</h1>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><router-link to="/admin/dashboard">Dashboard</router-link></li> 
          <li class="breadcrumb-item">Forms</li>
          <li class="breadcrumb-item active" aria-current="page">Purchase Logs</li>
        </ol>  
      </div>
      <div class="row">
        <div class="col-lg-12">
                ...
                <table class="table table-bordered table-striped" v-if="!loading && laravelData.data.length > 0">
                  ...
                  <tbody>
                    <tr v-for="log in laravelData.data" :key="log.id">
                        ...
                        <td>{{ log.token_amount }}</td>
                        <td>{{ log.polygon_amount }}</td>
                        <td>{{ log.date }}</td>
                        <td>{{ log.time }}</td>
                      <td>
  ...
  const fetchLogs = async (page = 1) => {
    loading.value = true;
    try {
      await axios.get(`/api/purchase-logs?page=${page}&term=${term.value}`, {
        headers: {
